...
const taskStatusOptions = computed(() =>
  Array.from(new Set(props.tasks.map((task) => task.status))).sort(),
)
const filteredTasks = computed(() => {
  ...
  return props.tasks.filter((task) => {
    ...
    const matchesQuery =
      ...
      task.inputPayload.prompt?.toLowerCase().includes(query) ||
      task.failureCode?.toLowerCase().includes(query)
      ...
      <template v-else>
        <el-descriptions :column="1" border>
          ...
          <el-descriptions-item label="Usage">
            <pre>{{
              JSON.stringify(selectedTask.usagePayload ?? null, null, 2)
            }}</pre>
          </el-descriptions-item>
